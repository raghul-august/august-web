...
export default function PreventionWellnessByLetterClient({
  initialData,
  letter,
  decodedLetter,
  language,
  ...
  useEffect(() => {
    logger.info('Initial prevention-wellness data:', JSON.stringify(initialData.items, null, 2));
  ...
  const prefetchNextPage = useCallback(async (nextPage) => {
    if (nextPage <= pagination.totalPages && !prefetchedData.current[nextPage]) {
      try {
        const data = await fetchByLetter('prevention-wellness', letter, language, nextPage);
  ...
  const handlePageChange = async (page) => {
    ...
    try {
      if (prefetchedData.current[page]) {
        ...
        const data = await fetchByLetter('prevention-wellness', letter, language, page);
        setItems(data.items);
        setPagination(data.pagination);
      }
    } catch (error) {
          ...
          <Grid container spacing={6} alignItems="center">
            ...
            <Grid item xs={12} md={6}>
              <Box sx={{
                ...
                <AlphabetGrid baseUrl={`/${language}/prevention-wellness`} />
              </Box>
            </Grid>
          </Grid>
        </Container>
          ...
          {error ? (
            ...
            <Box sx={{ textAlign: 'center', py: 8 }}>
              <Typography variant="h5" component="h2" gutterBottom>
                {langStrings.noPreventionWellnessFound || 'No topics found'}
              </Typography>
              <Typography variant="body1" color="text.secondary">
                {langStrings.tryAnother || 'Try another letter'}
              </Typography>
            </Box>
          )}
        </Container>
