...
export default function MentalHealthByLetterClient({
  initialData,
  letter,
  decodedLetter,
  language,
  ...
  useEffect(() => {
    logger.info('Initial mental-health data:', JSON.stringify(initialData.items, null, 2));
  ...
  const prefetchNextPage = useCallback(async (nextPage) => {
    if (nextPage <= pagination.totalPages && !prefetchedData.current[nextPage]) {
      try {
        const data = await fetchByLetter('mental-health', letter, language, nextPage);
  ...
  const handlePageChange = async (page) => {
    ...
    try {
      if (prefetchedData.current[page]) {
        ...
        const data = await fetchByLetter('mental-health', letter, language, page);
        setItems(data.items);
        setPagination(data.pagination);
      }
    } catch (error) {
          ...
          <Grid container spacing={6} alignItems="center">
            ...
            <Grid item xs={12} md={6}>
              <Box sx={{
                ...
                <AlphabetGrid baseUrl={`/${language}/mental-health`} />
              </Box>
            </Grid>
          </Grid>
        </Container>
          ...
          {error ? (
            ...
            <Box sx={{ textAlign: 'center', py: 8 }}>
              <Typography variant="h5" component="h2" gutterBottom>
                {langStrings.noMentalHealthFound || 'No topics found'}
              </Typography>
