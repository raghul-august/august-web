...
export default function MedicationsByLetterClient({
  initialData,
  letter,
  decodedLetter,
  language,
  ...
  const [medications, setMedications] = useState(initialData.items);
  ...
  const prefetchNextPage = useCallback(async (nextPage) => {
    if (nextPage <= pagination.totalPages && !prefetchedData.current[nextPage]) {
      try { 
        const data = await fetchByLetter('medications', letter, language, nextPage);
  ...
  const handlePageChange = async (page) => {
    ...
    try {
      ...
      const data = await fetchByLetter('medications', letter, language, page);
      setMedications(data.items);
      setPagination(data.pagination);
    } 
  } catch (error) {
        ...
        <Container maxWidth="lg">
          <Grid container spacing={6} alignItems="center">
            <Grid item xs={12} md={6}>
              ...
              >
                {langStrings.medicationsDescription}
              </Typography>
              <SearchBar 
                placeholder={langStrings.medicationsSearchPlaceholder} 
                indices={{
                  health_library: process.env.NEXT_PUBLIC_ALGOLIA_INDEX_NAME || 'Health_Library',
                }}
                tags={['health_library', 'medications']} 
              />
            </Grid>
            ...
            <Grid item xs={12} md={6}>
              <Box sx={{
                ...
                <AlphabetGrid baseUrl={`/${language}/medications`} />
              </Box>
            </Grid>
          </Grid>
        </Container>
      ...
      <Box sx={{ backgroundColor: 'white', py: 4, position: 'relative' }}>
            ...
            <Box sx={{ textAlign: 'center', py: 8 }}>
              <Typography variant="h5" component="h2" gutterBottom>
                {langStrings.noMedicationsFoundMessage}
              </Typography>
