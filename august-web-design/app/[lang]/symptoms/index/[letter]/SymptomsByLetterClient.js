...
export default function SymptomsByLetterClient({
  initialData,
  symptoms,
  letter,
  decodedLetter,
  ...
  metaIndexTitle,
  metaDescription,
  noSymptomsFoundMessage,
  error,
  isWebviewSource: hideDownloadButtonFromServer = false,
}) {
  const [symptomsitem, setSymptoms] = useState(initialData.items);
  ...
  const prefetchNextPage = useCallback(async (nextPage) => {
    if (nextPage <= pagination.totalPages && !prefetchedData.current[nextPage]) {
      try {
        const data = await fetchByLetter('symptoms', letter, language, nextPage);
  ...
  const handlePageChange = async (page) => {
    ...
    try {
      ...
      const data = await fetchByLetter('symptoms', letter, language, page);
      setSymptoms(data.items);
      setPagination(data.pagination);
    } 
  } catch (error) {
          ...
          <Grid container spacing={6} alignItems="center">
            ...
            <Grid item xs={12} md={6}>
              ...
              >
                ...
                <AlphabetGrid baseUrl={`/${language}/symptoms`} />
              </Box>
            </Grid>
          </Grid>
        </Container>
