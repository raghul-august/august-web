...
export const getAvailableLanguagesForMentalHealth = cache(async function getAvailableLanguagesForMentalHealth(handle) {
  const result = await query(
    `SELECT DISTINCT language_id
     FROM mental_health_translations_new
     WHERE id = (SELECT id FROM mental_health_new WHERE handle = $1)`,
    [handle]
  );
...
export const getMentalHealthBySlug = cache(async function getMentalHealthBySlug(slug, lang = 'en') {
  ...
  const result = await query(
    ...
    m.image,
    m.status,
    CASE WHEN mt.record_id IS NOT NULL THEN true ELSE false END as has_translation,
    (SELECT array_agg(DISTINCT hb.component_key) FROM health_library_banners hb LEFT JOIN health_library_banner_assignments hba ON hb.id = hba.banner_id WHERE hb.status = 'active' AND (hba.mental_health_id = m.id OR hb.audience = 'unconditional')) as banner_keys
FROM
    mental_health_new m
LEFT JOIN
    mental_health_translations_new mt ON m.id = mt.id AND mt.language_id = $2
LEFT JOIN
...
export async function getMentalHealthByLetter(letter, lang = 'en', page = 1, limit = 20) {
  const languageId = languageMap[lang] || 1;
...
LEFT JOIN mental_health_translations_new mt ON m.id = mt.id AND mt.language_id = $2
