...
import type { PsychopathyResult } from "@/app/utils/tools/psychopathy-test-scoring";
import { PSYCHOPATHY_TIERS } from "@/app/data/tools/psychopathy-test-config";
import QuizContainer from "../shared/QuizContainer";
import ScoreRing from "../shared/ScoreRing";
import TierBadge from "../shared/TierBadge";

interface ResultsScreenProps {
  result: PsychopathyResult;
  onRestart: () => void;
}

export default function ResultsScreen({ result, onRestart }: ResultsScreenProps) {
  const handleTalkToAugust = useCallback(() => {
    trackToolEvent("psychopathy-test", "cta_clicked", {
    ...
    openAugustChat(
      `Hi, I just took the Psychopathy self-test and scored ${result.score}/${result.maxScore} (${result.tier.label}). I'd like to talk about what this means and what good next steps look like.`,
    );
  }, [result]);
    ...
    <QuizContainer showFooter={true}>
      <div className="bg-linear-to-b from-(--surface-subtle) to-(--brand-subtle)/40">
        <div className="max-w-160 mx-auto px-5 pt-10 pb-15">
          ...
          >
            <p className="text-base font-medium text-(--text-tertiary) tracking-[0.08em] mb-5 uppercase">
          ...
          >
            ...
            <p className="text-[0.85rem] text-(--text-tertiary) mb-4 leading-[1.65]">
              ...
              Strongly Agree). Five items are reverse-keyed so that agreement
              with empathic or conscientious statements doesn’t inflate your
              score. The total falls between 20 and 100 and is banded as:
            </p>
            <ul className="flex flex-col gap-2 m-0 p-0 list-none">
              {PSYCHOPATHY_TIERS.map((tier) => (
                <li
                  key={tier.id}
          ...
          >
            ...
            <p className="text-[0.88rem] leading-[1.75] text-(--text-secondary) mb-2">
              A self-score isn’t a diagnosis. Psychopathy researchers know
              that self-report has real limits — people very high on the
              construct sometimes under-report (deliberate impression
              management) and sometimes over-report (they admire the traits).
              The clinical gold-standard, the PCL-R, is a structured interview
              with collateral information.
            </p>
            <p className="text-[0.88rem] leading-[1.75] text-(--text-secondary) m-0">
              If the description here matches your experience and the
              patterns are causing harm — to you or the people around you — a
              licensed mental health provider is the right next step. You can
              also chat with August inside this page to talk through what
              your score means without committing to anything.
            </p>
          </motion.div>
          ...
          <div className="text-center text-[0.72rem] leading-[1.65] text-(--text-tertiary) opacity-85 max-w-140 mx-auto px-4">
            <p className="tl-disclaimer">
              ...
              adapted from the PCL-R, PPI, and Levenson SRP for
              self-reflection only — share these results with a clinician for
              a real evaluation.
            </p>
          </div>
        </div>
