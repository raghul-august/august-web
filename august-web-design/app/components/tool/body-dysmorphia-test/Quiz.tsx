...
import {
  computeBodyDysmorphiaResult,
  bodyDysmorphiaScoreBucket,
  type BodyDysmorphiaAnswers,
  type BodyDysmorphiaResult,
} from "@/app/utils/tools/body-dysmorphia-test-scoring";
...
export default function Quiz({ afterContent }: { afterContent?: ReactNode }) {
  ...
  useEffect(() => {
    ...
    trackToolEvent("body-dysmorphia-test", "viewed");
  }, []);
  ...
  const handleStartTest = useCallback(() => {
    track("body_dysmophia_test_started", {
      event_category: "Body Dysmorphia",
      total_questions: totalQuestions,
    });
    trackToolEvent("body-dysmorphia-test", "started");
    setAnswers({});
    setCurrentIndex(0);
    timer.reset();
    setScreen("question");
  ...
  const handleAnswer = useCallback(
    (value: number) => {
      ...
      track("body_dysmophia_test_question_answered", {
        event_category: "Body Dysmorphia",
  ...
  const result = useMemo<BodyDysmorphiaResult>(
    () => computeBodyDysmorphiaResult(answers as BodyDysmorphiaAnswers),
    [answers],
  );

  useEffect(() => {
    if (screen !== "results") return;
    const sig = `${bodyDysmorphiaScoreBucket(result.score)}|${result.tier.id}`;
    if (sig === completedRef.current) return;
    completedRef.current = sig;
    trackToolEvent("body-dysmorphia-test", "completed", {
      score: result.score,
      score_bucket: bodyDysmorphiaScoreBucket(result.score),
      tier: result.tier.id,
  ...
  if (screen === "question") {
    ...
    return (
      <QuestionScreen
        ...
        selectedAnswer={(answers as BodyDysmorphiaAnswers)[current.id] ?? null}
