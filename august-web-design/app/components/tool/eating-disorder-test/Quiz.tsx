...
import {
  computeEatingDisorderResult,
  eatingDisorderScoreBucket,
  type EatingDisorderAnswers,
  type EatingDisorderResult,
} from "@/app/utils/tools/eating-disorder-test-scoring";
...
export default function Quiz({ afterContent }: { afterContent?: ReactNode }) {
  ...
  useEffect(() => {
    ...
    trackToolEvent("eating-disorder-test", "viewed");
  }, []);
  ...
  const handleStartTest = useCallback(() => {
    track("eating_disorder_test_started", {
      event_category: "Eating Disorder Test",
      total_questions: totalQuestions,
    });
    trackToolEvent("eating-disorder-test", "started");
    setAnswers({});
    setCurrentIndex(0);
    timer.reset();
    setScreen("question");
  ...
  const handleAnswer = useCallback(
    (value: number) => {
      ...
      track("eating_disorder_test_question_answered", {
        event_category: "Eating Disorder Test",
  ...
  const result = useMemo<EatingDisorderResult>(
    () => computeEatingDisorderResult(answers as EatingDisorderAnswers),
    [answers],
  );

  useEffect(() => {
    if (screen !== "results") return;
    const sig = `${eatingDisorderScoreBucket(result.score)}|${result.tier.id}|${result.primaryDomain.id}`;
    if (sig === completedRef.current) return;
    completedRef.current = sig;
    trackToolEvent("eating-disorder-test", "completed", {
      score: result.score,
      score_bucket: eatingDisorderScoreBucket(result.score),
      tier: result.tier.id,
      primary_domain: result.primaryDomain.id,
      functional_impact: result.functionalLabel ?? "unanswered",
    });
  }, [screen, result]);
  ...
  if (screen === "question") {
    ...
    return (
      <QuestionScreen
        ...
        selectedAnswer={(answers as EatingDisorderAnswers)[current.id] ?? null}
