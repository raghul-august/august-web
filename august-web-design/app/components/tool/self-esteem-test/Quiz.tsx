...
import {
  computeSelfEsteemResult,
  selfEsteemScoreBucket,
  type SelfEsteemAnswers,
  type SelfEsteemResult,
} from "@/app/utils/tools/self-esteem-test-scoring";
...
export default function Quiz({ afterContent }: { afterContent?: ReactNode }) {
  ...
  useEffect(() => {
    ...
    trackToolEvent("self-esteem-test", "viewed");
  }, []);
  ...
  const handleStartTest = useCallback(() => {
    track("self_esteem_test_started", {
      event_category: "Self-Esteem Test",
      total_questions: totalQuestions,
    });
    trackToolEvent("self-esteem-test", "started");
    setAnswers({});
    setCurrentIndex(0);
    timer.reset();
    setScreen("question");
  ...
  const handleAnswer = useCallback(
    (value: number) => {
      ...
      track("self_esteem_test_question_answered", {
        event_category: "Self-Esteem Test",
  ...
  const result = useMemo<SelfEsteemResult>(
    () => computeSelfEsteemResult(answers as SelfEsteemAnswers),
    [answers],
  );

  useEffect(() => {
    if (screen !== "results") return;
    const sig = `${selfEsteemScoreBucket(result.score)}|${result.tier.id}`;
    if (sig === completedRef.current) return;
    completedRef.current = sig;
    trackToolEvent("self-esteem-test", "completed", {
      score: result.score,
      score_bucket: selfEsteemScoreBucket(result.score),
      tier: result.tier.id,
  ...
  if (screen === "question") {
    ...
    return (
      <QuestionScreen
        ...
        selectedAnswer={(answers as SelfEsteemAnswers)[current.id] ?? null}
