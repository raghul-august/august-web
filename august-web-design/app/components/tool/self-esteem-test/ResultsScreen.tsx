...
import type { SelfEsteemResult } from "@/app/utils/tools/self-esteem-test-scoring";
import { SELF_ESTEEM_TIERS } from "@/app/data/tools/self-esteem-test-config";
import QuizContainer from "../shared/QuizContainer";
import ScoreRing from "../shared/ScoreRing";
import TierBadge from "../shared/TierBadge";

interface ResultsScreenProps {
  result: SelfEsteemResult;
  onRestart: () => void;
}

export default function ResultsScreen({ result, onRestart }: ResultsScreenProps) {
  const handleTalkToAugust = useCallback(() => {
    trackToolEvent("self-esteem-test", "cta_clicked", {
    ...
    openAugustChat(
      `Hi, I just took the Self-Esteem self-test and scored ${result.score}/${result.maxScore} (${result.tier.label}). I'd like to talk about what this means and what good next steps look like.`,
    );
  }, [result]);
        ...
        <div className="max-w-160 mx-auto px-5 pt-10 pb-15">
          ...
          >
            ...
            <p className="text-[0.85rem] text-(--text-tertiary) mb-4 leading-[1.65]">
              ...
              Strongly Agree). Six items are reverse-keyed so that agreeing
              with a negative self-statement counts as lower self-esteem. Your
              total falls between 20 and 100 and is banded as:
            </p>
            <ul className="flex flex-col gap-2 m-0 p-0 list-none">
              {SELF_ESTEEM_TIERS.map((tier) => (
                <li
                  key={tier.id}
          ...
          >
            ...
            <p className="text-[0.88rem] leading-[1.75] text-(--text-secondary) mb-2">
              Self-esteem moves. It is not a fixed personality trait — it
              responds to how you talk to yourself, the evidence you collect
              about your own competence, and the people you spend time with.
              Cognitive behavioural therapy and Kristin Neff’s self-compassion
              programme both have strong evidence for shifting this score
              upward.
            </p>
            <p className="text-[0.88rem] leading-[1.75] text-(--text-secondary) m-0">
              If you want a low-pressure place to start, chat with August. It
              is an AI health companion that can help you unpack the result,
              spot the patterns driving it, and pick a useful next step.
            </p>
          </motion.div>
          ...
          <div className="text-center text-[0.72rem] leading-[1.65] text-(--text-tertiary) opacity-85 max-w-140 mx-auto px-4">
            <p className="tl-disclaimer">
              ...
              adapted from the Rosenberg, Coopersmith, Robson, and Neff scales
              for self-reflection only — share these results with a clinician
              for any real evaluation.
            </p>
          </div>
        </div>
      </div>
