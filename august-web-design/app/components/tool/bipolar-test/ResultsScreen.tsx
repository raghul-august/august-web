...
import type { BipolarResult } from "@/app/utils/tools/bipolar-test-scoring";
...
const BANDS = [
  ...
  { range: "68–83", label: "Significant" },
  { range: "84–100", label: "Very high" },
];
...
export default function ResultsScreen({ result, onRestart }: ResultsScreenProps) {
  const handleTalkToAugust = useCallback(() => {
    ...
    // });
    trackToolEvent("bipolar-test", "cta_clicked", {
      cta_type: "talk_to_august",
      tier: result.tier.id,
    });
    openAugustChat(
      `Hi, I just took the Bipolar self-test and scored ${result.score}/${result.maxScore} (${result.tier.label}). I'd like to talk about what this means and what good next steps look like.`,
    );
  }, [result]);

  const { resultRef, handleDownload } = useDownloadResult({
    toolId: "bipolar-test",
    filename: `bipolar-test-${result.tier.label}`,
    heading: "Bipolar Test Result",
    subtitle: `Bipolar Report • ${result.tier.label}`,
    toolName: "Bipolar Test",
    maxPageHeight : 1400
  });
        ...
        <div ref={resultRef} className="max-w-[640px] mx-auto px-8 sm:px-12 pt-10 pb-[60px]">
            ...
            <div className="flex-1">
              ...
              <p className="text-[0.85rem] leading-[1.65] text-(--text-secondary) m-0">
                {result.positiveScreen ? (
                  <>
                    Scores of <strong>{result.positiveScreenCutoff} or higher </strong>
                    sit at or above the moderate band, a level where a fuller
                    assessment with a mental health professional tends to be
                    worth it.
                  </>
                ) : (
                  <>
                    Scores below <strong>{result.positiveScreenCutoff}</strong>{" "}
                    fall in the minimal-to-mild range. If specific mood swings,
                    energy spikes, or low periods still feel hard, that
                    doesn&apos;t mean nothing&apos;s there it just means this
                    brief screen didn&apos;t flag a moderate or higher pattern
                    overall.
                  </>
                )}
              </p>
          ...
          >
            ...
            <p className="text-[0.85rem] text-(--text-tertiary) mb-4 leading-[1.65]">
              Each of the {result.total} items is rated 1 (strongly disagree) to
              5 (strongly agree). Three items are reverse-scored agreeing with
              them indicates fewer bipolar-spectrum signs. Adding them up gives
              your total ({result.minScore}–{result.maxScore}). The 5-tier
              banding is:
            </p>
          ...
          >
            ...
            <p className="text-[0.88rem] leading-[1.75] text-(--text-secondary) mb-2">
              Bipolar disorder is highly treatable once recognized. Mood
              stabilizers (such as lithium or lamotrigine), structured therapy
              (CBT, interpersonal and social rhythm therapy), and a stable
              sleep–wake routine all have strong evidence behind them. Early
              identification tends to make the path forward smoother.
            </p>
            <p className="text-[0.88rem] leading-[1.75] text-(--text-secondary) m-0">
              ...
              you understand the result, and point you toward the right kind of
              support.
            </p>
          </motion.div>
