...
import type { AnxietyResult } from "@/app/utils/tools/anxiety-test-scoring";
...
export default function ResultsScreen({ result, onRestart }: ResultsScreenProps) {
  const handleTalkToAugust = useCallback(() => {
    trackToolEvent("anxiety-test", "cta_clicked", {
      cta_type: "talk_to_august",
      tier: result.tier.id,
      score : result.score,
      positive_screen: result.positiveScreen,
      positiveScreenCutoff: result.positiveScreenCutoff
    });
    openAugustChat(
      `Hi, I just took the Anxiety self-test and scored ${result.score}/${result.maxScore} (${result.tier.label}). I'd like to talk about what this means and what good next steps look like.`,
    );
  }, [result]);

  const { resultRef, handleDownload } = useDownloadResult({
    toolId: "anxiety-test",
    filename: `anxiety-test-${result.tier.label}`,
    heading: "Anxiety Test Result",
    subtitle: `Anxiety Report • ${result.tier.label}`,
    toolName: "Anxiety Test",
    maxPageHeight : 1330
  });
        ...
        <div ref={resultRef} className="max-w-[640px] mx-auto px-8 sm:px-12 pt-10 pb-[60px]">
          ...
          >
            ...
            <p className="text-[0.85rem] text-(--text-tertiary) mb-4 leading-[1.65]">
              Each of the {result.total} items is rated 1 (strongly disagree) to 5
              (strongly agree). Four items are reverse-scored agreeing with them
              indicates lower anxiety. Adding them up gives your total
          ...
          >
            ...
            <p className="text-[0.88rem] leading-[1.75] text-(--text-secondary) mb-2">
              Generalized anxiety is highly treatable. Cognitive-behavioural
              therapy (CBT) has the strongest evidence base, and many people
              see significant improvement with focused treatment. Medication
              (typically SSRIs or SNRIs) can help in some cases, alongside
              therapy.
            </p>
