...
export async function POST(request: Request) {
    ...
    if (!baseUrl || !authToken) {
        logger.error('[subscription-creation] missing env', {
            hasBaseUrl: !!baseUrl,
            hasToken: !!authToken,
        });
        return NextResponse.json(
            { valid: false, reason: 'payment_subscription_creation_failed' },
            { status: 500 }
        );
    }

    let body: { token?: string; payment_plan_id?: string; pincode?: string };
    try {
    ...
    if (!pincode) {
        return NextResponse.json(
            { valid: false, reason: 'pincode_required' },
            { status: 400 }
        );
    }
    ...
    try {
        const upstream = await fetch(
            `${baseUrl}/s/payments/${encodeURIComponent(slug)}/checkout/${encodeURIComponent(token)}/subscription-creation`,
            {
                method: 'POST',
                headers: forwarded,
                body: JSON.stringify({ payment_plan_id: paymentPlanId, pincode }),
                cache: 'no-store',
            }
        );
        ...
        logger.error('[subscription-creation] fetch failed', { err: String(err) });
        return NextResponse.json(
            { valid: false, reason: 'payment_subscription_creation_failed' },
            { status: 502 }
        );
